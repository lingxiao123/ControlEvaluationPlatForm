...
namespace PlatFormWeb.Role
{
    public partial class UserRole_List :PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                btnImport.OnClientClick = Window1.GetShowReference("~/Role/UserRole_Set.aspx");
                BindGrid();
            }
        }
        ...
        private DataTable GetPagedDataTable()
        {
            ...
            string sql = "select ur.GroupCode, ut.UserName,r.GroupName from UserGroup as ur left join UserTable as ut  on ur.UsedId=ut.Id left join GroupInfo as r on ur.GroupCode=r.GroupCode";
            DataTable dt = DBAccess.QueryDataTable(sql);
